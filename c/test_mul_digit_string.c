...
int main()
{
  ...
  int status = call_mul_digit_string(0, scalars, numeric_input, numeric_output);
  if (!status)
    status = call_mul_digit_string(1, scalars, numeric_input, numeric_output);

  return(status);
}

int call_mul_digit_string(const int index, const int *scalars, const char **numeric_input, const char **numeric_output)
{
  int a = scalars[index];
