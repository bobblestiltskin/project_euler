...
pe013(J, S, L) when J =< 10 ->
%  io:format("j is ~w, s is ~w, l is ~w~n", [J, S, L]),
  pe013(J + 1, (10 * S) + colsum(J), 0);
    
pe013(J, S, L) ->
  Last = lists:sublist(integer_to_list(S div num()), 10),
%  io:format("j is ~w, s is ~w, l is ~w, last is ~w~n", [J, S, L, Last]),
  if 
    Last == L ->
      io:format("~w~n", [list_to_integer(Last)]);
    true ->      
      pe013(J + 1, (10 * S) + colsum(J), Last)
  end.
    
colsum(J) ->
  colsum(rows(), J, 0).

colsum(I, _J, Sum) when I == 0 ->
%  io:format("J is ~w and ~w~n", [_J, Sum]),
  Sum;
  
colsum(I, J, Sum) ->
  colsum(I - 1, J, Sum + get_element(I, J)).
