...
# append takes list ptr in x0, read_only pointer in x1
# reads all bytes from the list pointed at by read only pointer
# and writes all to the x0 list_ptr
append:
        stp	fp, lr, [sp, #-0x10]!
        mov	fp, sp
...
# vqsort sorts the countnames vector
...
# after setting the less, equal and greater lists need to push to the stack since
# we recurse and then retrieve when we come back out
...
# compares x0 with x1 [len is first byte] - returns 0 in x0 if equal
...
loopend:
	...
	cmp	wnsize, 0
	b.eq	qsort_partition_complete

	b	qiloop
qsort_partition_complete:
	ldr	x0, =less
	bl	vectostack
	...
	mov	x2, equal_ptr
	mov	x3, greater_ptr
# recurse and watch the fireworks
	bl	vqsort
	mov	less_ptr, x0
