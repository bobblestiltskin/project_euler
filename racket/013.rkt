...
(define (add-numbers)
  (for/fold ([nstring "000000000000"][result_num 0]) ([j (in-range 0 width)])
    #:break
    (let* ([tmpstring (number->string (+ (* result_num 10) (colsum j)))]
           [tlen (sub1 (string-length tmpstring))]
           [slen (min 10 tlen)]
           [tshort (substring tmpstring 0 slen)]
           [nshort (substring nstring 0 slen)]
           [new (+ (* result_num 10) (colsum j))]
          )
      (and (> tlen 10) (string=? tshort nshort))
    )
    (let ([new (+ (* result_num 10) (colsum j))])
      (values (number->string new) new)
    )
  )
)
