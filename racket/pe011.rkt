...
(define (north i j)
  (if (< i (sub1 lsize))
    0
    (for/fold ([eprod 1]) ([k (in-range 0 lsize)])
      (* (vector-ref (vector-ref tmat (- i k)) j) eprod)
    )
  )
)

(define (northeast i j)
  (if (or (< i (sub1 lsize)) (> j (- sqsize lsize))) 
    0
    (for/fold ([eprod 1]) ([k (in-range 0 lsize)])
      (* (vector-ref (vector-ref tmat (- i k)) (+ j k)) eprod)
    )
  )
)
(define (east i j)
  (if (> j (- sqsize lsize))
    0
    (for/fold ([eprod 1]) ([k (in-range 0 lsize)])
      (* (vector-ref (vector-ref tmat i) (+ j k)) eprod)
    )
  )
)

(define (southeast i j)
  (if (or (> i (- sqsize lsize)) (> j (- sqsize lsize))) 
    0
    (for/fold ([eprod 1]) ([k (in-range 0 lsize)])
      (* (vector-ref (vector-ref tmat (+ i k)) (+ j k)) eprod)
    )
...
(define (southwest i j)
  (if (or (> i (- sqsize lsize)) (< j (sub1 lsize))) 
    0
    (for/fold ([eprod 1]) ([k (in-range 0 lsize)])
      (* (vector-ref (vector-ref tmat (+ i k)) (- j k)) eprod)
    )
...
(define (northwest i j)
  (if (or (< i (sub1 lsize)) (< j (sub1 lsize))) 
    0
    (for/fold ([eprod 1]) ([k (in-range 0 lsize)])
      (* (vector-ref (vector-ref tmat (- i k)) (- j k)) eprod)
    )
