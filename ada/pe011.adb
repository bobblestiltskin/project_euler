...
function compute_point(a: Matrix; i : Rows; j : Cols) return Integer is
  amax : Integer := 0;
  n : Integer := 0;
  begin
-- north 
...
begin
  ...
  for i in 3 .. 22 loop
    for j in 3 .. 22 loop
      point := compute_point(a, Rows(i), Cols(j));
  ...
  Put (Ada.Strings.Fixed.Trim(Integer'Image(amax), Side => Both));
end pe011;
